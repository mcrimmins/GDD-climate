...
#   tempMap<- mask(x = unC$map, mask = USA)
#  rc <- reclassify(tempMap, rclmat)
# #
#    zonePoly<-rasterToPolygons(rc, n=4, na.rm=TRUE, digits=12, dissolve=TRUE)
#    proj4string(zonePoly) <- "+proj=longlat +datum=WGS84"
#    library(rgdal)
#    writeOGR(zonePoly, ".", "./clusterTrends/threeZone", driver="ESRI Shapefile")
# # ------
...
# # cluster histograms
# # approach 3
# z <- layerize(unC$map, falseNA=TRUE)
# sz <- z[[9]] * gdd50raw_x4[[69]]
# x <- hist(sz) #hist(sz, breaks=c(0,5,15,30,45,65))
# f3 <- sapply(x, function(i) i$counts)
# boxplot(gdd50raw_x4,z[[1]])


# # get zonal mean of each cluster
zStats<-as.data.frame(t(zonal(allGddraw, unC$map, 'mean'))) # or classMap; mean or median
zStats<-zStats[-1,]
...
#   layer(sp.polygons(states))
 pMap<-levelplot(classMap, col.regions=darkcols, par.settings=list(panel.background=list(col="white")),
           ...
           width=1, height=0.9,labels=list(cex=0.38, font=1, col="black")))+
...
# TO DO
# - define regions on map - west, east, central
# - need prediction interval on trend lines?
# 

# with viewport
# library(grid)
# vp <- viewport(width = 5, height = 5, x = 2, y = 2)
# #Just draw the plot twice
# png("/home/crimmins/RProjects/TopoWx/clusterTrends/figs/fig2TS.png", width = 7, height = 5, units = "in", res = 300L)
# print(pTS, newpage = FALSE)
# print(pMap, vp = vp)
# dev.off()


# library(metaplot)
# multiplot(pTS,pMap, nrow = 2)
