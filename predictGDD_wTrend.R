...
gdd50raw<-stack("./fixed/X4_rawDOY_baseT10_thresh50.grd")
gdd250raw<-stack("./fixed/X4_rawDOY_baseT10_thresh250.grd")
gdd450raw<-stack("./fixed/X4_rawDOY_baseT10_thresh450.grd")
...
gdd50_x4 <- mask(gdd50raw, maskNA)
gdd250_x4 <- mask(gdd250raw, maskNA)
gdd450_x4 <- mask(gdd450raw, maskNA)
...
# models and predictions 50 to 450
  ...
  #gdd.model = lm(gdd450 ~ gdd50+years, data=gddTS) # y~x
  ...
  #effect_plot(gdd.model, pred = gdd50, interval = TRUE, plot.points = TRUE)
    #new.gdd = data.frame(gdd50=125, years=2018)
    new.gdd = data.frame(gdd250=146)
